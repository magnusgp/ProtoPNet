...
def push_prototypes(dataloader,  # pytorch dataloader (must be unnormalized in [0,1])
    ...
    prototype_network_parallel.module.prototype_vectors.data.copy_(torch.tensor(prototype_update, dtype=torch.float32).cuda() if torch.cuda.is_available() else torch.tensor(prototype_update, dtype=torch.float32))
    ...
    for j in range(n_prototypes):
        ...
        if batch_min_proto_dist_j < global_min_proto_dist[j]:
            ...
            if dir_for_saving_prototypes is not None:
                output_dir = os.path.join(dir_for_saving_prototypes)
                makedir(output_dir)
