...
def find_k_nearest_patches_to_prototypes(dataloader,  # pytorch dataloader (must be unnormalized in [0,1])
    ...
    for search_batch_input, search_y in tqdm(dataloader, total=len(dataloader)):
        ...
        for img_idx, distance_map in enumerate(proto_dist_):
            for j in range(n_prototypes):
                ...
                if len(heaps[j]) < k:
                    ...
                    # heappushpop runs more efficiently than heappush followed by heappop
                    heapq.heappushpop(heaps[j], closest_patch)

    # after looping through the dataset every heap will have the k closest prototypes
    for j in range(n_prototypes):
        # finally sort the heap; the heap only contains the k closest but they are not ranked yet
        heaps[j].sort()
        heaps[j] = heaps[j][::-1]
        ...
        if full_save:
            ...
            for i, patch in enumerate(heaps[j]):
                ...
                np.save(os.path.join(dir_for_saving_images, 'nearest-' + str(i+1) + '_act.npy'),
                        patch.act_pattern)
                ...
                np.save(os.path.join(dir_for_saving_images, 'nearest-' + str(i+1) + '_high_act_patch_indices.npy'),
            ...
            with open(os.path.join(dir_for_saving_images, 'prototype_class_id.txt'), 'w') as f:
    ...
    if full_save:
        ...
        with open(os.path.join(dir_for_saving_images, 'prototype_full_class_id.txt'), 'w') as f:
            f.write('\n'.join(labels.astype(str).tolist()))
